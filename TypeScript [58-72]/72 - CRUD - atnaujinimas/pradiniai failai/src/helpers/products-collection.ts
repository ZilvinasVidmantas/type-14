...
class ProductsCollection {
  ...
  public createProduct = ({ categoriesIds, ...productProps }: ProductData) => {
  ...
  public findProductDataById = (productId: string): ProductData => {
    const foundProduct = this.privateProducts.find((product) => product.id === productId);
    if (foundProduct === undefined) {
      throw new Error(`Produktas su id "${productId}" nerastas`);
    }
  ...
  public updateProduct = ({ id, categoriesIds, ...productProps }: UpdateProductData) => {
    const foundProduct = this.privateProducts.find((product) => product.id === id);

    if (foundProduct === undefined) {
      throw new Error(`Produktas su id "${id}" nerastas`);
    }

    foundProduct.title = productProps.title;
    foundProduct.price = productProps.price;
    if (productProps.description) {
      foundProduct.description = productProps.description;
    }

    this.privateProductsCategories = this.privateProductsCategories
      .filter(({ productId }) => productId !== id);

    const newProductProductsCategories = categoriesIds.map((categoryId) => ({
      id: createId(),
      productId: id,
      categoryId,
    }));
