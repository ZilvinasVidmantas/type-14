...
console.group('2. Naudojant "get" ir "set" ES6 funkcijas:');
{
  class Person {
    ...
    private privateItems!: Array<{ title: string; price: number; }>;
    ...
    set items(val: Array<{ title: string, price: number }>) {
      ...
      this.privateItems = JSON.parse(JSON.stringify(val));
    }

    get items() {
      return JSON.parse(JSON.stringify(this.privateItems));
    }
